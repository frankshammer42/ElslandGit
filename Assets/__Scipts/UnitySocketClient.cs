...
//        StartCoroutine(DisableParticle());
    ...
    void OnIntroBlockErased(Socket socket, Packet packet, params object[] args){
        ...
        if (_numberOfBlocksErased == _numberOfConnections){
    ...
    void onCut(Socket socket, Packet packet, params object[] args){
        ...
        Vector2D pointA = new Vector2D(cutPoints[cutIndex*4], cutPoints[cutIndex*4+1]);
        Vector2D pointB = new Vector2D(cutPoints[cutIndex*4+2], cutPoints[cutIndex*4+3]);
        DrawLine(pointA, pointB, cutLineColor);
        pointA.x = pointA.x + xTrainOffset;
        pointA.y = pointA.y + yTrainOffset;
        pointB.x = pointB.x + xTrainOffset;
        pointB.y = pointB.y + yTrainOffset;
...
//        List<Slice2D> sliceResult = Slicer2D.LinearSliceAll(cutPair, Slice2DLayer.Create());
    ...
    private void DrawLine(Vector2D start2D, Vector2D end2D, Color color){
        Vector3 start = new Vector3((float)start2D.x + xTrainOffset, (float)start2D.y + yTrainOffset, (float)-122);
        Vector3 end = new Vector3((float)end2D.x + xTrainOffset, (float)end2D.y + yTrainOffset, (float)-122);
//        start = start * 1.2f;
//        end = end * 1.2f;
