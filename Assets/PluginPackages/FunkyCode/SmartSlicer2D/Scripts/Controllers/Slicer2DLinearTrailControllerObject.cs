...
public class Slicer2DLinearTrailControllerObject : Slicer2DControllerObject {
	public LinearSlicerTrail[] linearTrail = new LinearSlicerTrail[10];
	...
	public void Initialize() {
		for(int id = 0; id < 10; id++) {
			...
			linearTrail[id] = new LinearSlicerTrail();
		}
	}
		...
		for(int id = 0; id < trailRenderer.Length; id++) {
			...
			if (addForce) {
				foreach (Slice2D p in results)  {
					Slicer2DAddForce.LinearTrail(p, addForceAmount);
				}
			}
		}
	}
	...
	public void Draw(Transform transform) {
		...
		for(int id = 0; id < trailRenderer.Length; id++) {
			visuals.GenerateTrailMesh(input.GetInputPosition(id), linearTrail[id].trailList, transform);
		}
	...
	public void SetTrailPosition(TrailRenderer trail, int id) {
		Vector3 pos = input.GetInputPosition(id).ToVector3(); 
